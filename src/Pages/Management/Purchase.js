...
const ProductTable = () => {
  ...
  const handleAddRow = () => {
    ...
    //   let temp = { ...rowData, qty: data.quantity, rate_per_unit, tax_amount, cgst, sgst, total, gst: parseFloat(rowData.gst) };
    //   setRows([...rows, temp]);
    ...
    console.log("Hello : ", rowData);
    let rate_per_unit = parseFloat(data.rate_per_unit)/parseFloat(data.quantity);
    let tax_amount = parseFloat(data.rate_per_unit).toFixed(2);
    let cgst = parseFloat((tax_amount * parseFloat(rowData.gst)/100)/2).toFixed(2);
    let sgst = parseFloat((tax_amount * parseFloat(rowData.gst)/100)/2).toFixed(2);
    total = parseFloat(parseFloat(tax_amount) + parseFloat(cgst) + parseFloat(sgst)).toFixed(2);
    console.log("Total : ", tax_amount + cgst + sgst)
    let temp = { ...rowData, qty: data.quantity, rate_per_unit, tax_amount, cgst, sgst, total, gst: 0 };
    setRows([...rows, temp]);
    console.log("rows are", rowData)
    setBname([...bname, rowData])
    console.log("data to print", bname)
  ...
  // }, [rowData])
  const taxamnt = parseFloat(rows.reduce((accumulator, item) => accumulator + item.tax_amount, 0));
  const cgsttotal = parseFloat(rows.reduce((accumulator, item) => accumulator + item.cgst, 0));
  const sgsttotal = parseFloat(rows.reduce((accumulator, item) => accumulator + item.sgst, 0));
  const totalamnt = parseFloat(rows.reduce((accumulator, item) => accumulator + parseFloat(item.total), 0));
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      const response = axios.post(`${process.env.REACT_APP_API_URL}/makepurchase`, dataForRequest);
    ...
    rows.forEach((item) => {
      ...
      const formattedTaxableAmount = parseFloat(item.tax_amount).toFixed(2);
    ...
    taxableAmountToatal.textContent = parseFloat(taxamnt).toFixed(2);
            ...
            <TableBody>
              {rows.map((row, index) => (
                <TableRow key={index} onClick={() => { handleRow(row) }} >
                  ...
                  <TableCell>
                    <Typography>{parseFloat(row.tax_amount).toFixed(2)}</Typography>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Typography>{parseFloat(row.total)}</Typography>
                  </TableCell>
                  <TableCell>
                    <DeleteIcon
        ...
        >
          ...
          >
            ...
            <Box sx={{ width: "50%", height: "100%" }}>
              ...
              >
                {rows.map((item) => (
                  <Typography sx={{ fontSize: 15, m: 0.5 }}>{parseFloat(item.tax_amount).toFixed(2)}</Typography>
                ))}
              </Box>
              <Box
                sx={{
              ...
              >
                <Typography>{parseFloat(taxamnt).toFixed(2)}</Typography>
              </Box>
            </Box>
          </Box>
          <Box
            ...
            <Box sx={{ width: "33.3%", height: "100%", borderRight: 1 }}>
              ...
              >
                <Box sx={{ width: '50%', height: '100%', borderRight: 1, display: 'flex', flexDirection: 'column' }} >
                  {rows.map((item) => (
                    <Typography sx={{ fontSize: 15, m: 0.5 }}>{parseFloat(item.gst) / 2}</Typography>
                  ))}
                </Box>
